...
-spec {{method}}({{#input_stream}}atom()|reference(){{/input_stream}}{{^input_stream}}{{pb_module}}:'{{input}}'(){{/input_stream}}) ->
    {{^output_stream}}{{^input_stream}}{ok, {{pb_module}}:'{{output}}'()}{{/input_stream}}{{#input_stream}}{ok, GunPid :: pid(), Http2Ref:: reference(), PbModule :: module()}{{/input_stream}}{{/output_stream}}{{#output_stream}}{{^input_stream}}{ok, GunPid :: pid(), Http2Ref:: reference(), PbModule :: module()}{{/input_stream}}{{#input_stream}}{ok, GunPid :: pid(), Http2Ref:: reference(), PbModule :: module()}{{/input_stream}}{{/output_stream}}|{error,eetcd:eetcd_error()}.
{{method}}(Request) ->
    {{^output_stream}}{{^input_stream}}eetcd_stream:unary(Request, '{{input}}', <<"{{full_service_path}}">>, '{{output}}', {{pb_module}}){{/input_stream}}{{#input_stream}}eetcd_stream:new(Request, <<"{{full_service_path}}">>, {{pb_module}}){{/input_stream}}{{/output_stream}}{{#output_stream}}{{^input_stream}}eetcd_stream:new(Request, <<"{{full_service_path}}">>, {{pb_module}}){{/input_stream}}{{/output_stream}}.
